...
const char* ssid = "Galaxy S22"; // Ganti dengan SSID WiFi Anda
const char* password = "lmam9607"; // Ganti dengan password WiFi Anda
...
void setup() {
  Wire.begin(D2, D1); // Inisialisasi I2C
...
void loop() {
  ...
  String message = "{\"nim\":\"G.231.21.0176\",\"suhu\":" + String(t) + ",\"kelembaban\":" + String(h) + "}";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(nim) == "G.231.21.0176" || String(nim) == "all" || String(nim) == "ALL") { // Sesuaikan dengan nim Anda
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("G.231.21.0176", mqtt_user, mqtt_password)) {
